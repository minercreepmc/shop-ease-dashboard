...
export class CacheStorageService {
  ...
  public forKey(key: string): CacheStorageFacet {
    return new CacheStorageFacet(key, this.storage);
  }
...
export class CacheStorageFacet {
  private key: string;
  private storage: StorageWrapper;
...
class SessionStorageWrapper implements StorageWrapper {
  ...
  private loadFromCache(): void {
    try {
      ...
      console.warn(
        'SessionStorageWrapper was unable to read from SessionStorage API.',
      );
      console.error(error);
    }
  }
  ...
  private persistToCache(): void {
    ...
    this.debounceOutsideNgZone(() => {
      ...
      try {
        ...
        console.warn(
          'SessionStorageWrapper was unable to write to SessionStorage API.',
        );
        console.error(error);
      }
    });
